...
@section('content')
                                                ...
                                                <div class="col-md-3 form-group">
                                                    ...
                                                    <div class="input-group">
                                                        <select name="building" id="building"
                                                            class="form-control input-building appearance-none <?php echo $errors->first('building') ? 'is-invalid' : ''; ?>">
                                                ...
                                                <div class="col-md-3 form-group">
                                                    ...
                                                    <div class="input-group">
                                                        <select name="floor" id="floor"
                                                            class="form-control input-building appearance-none <?php echo $errors->first('floor') ? 'is-invalid' : ''; ?>">
                                                ...
                                                <div class="col-md-6 form-group">
                                                    ...
                                                    <div class="input-group">
                                                        <select name="room" id="room"
                                                            class="form-control input-building appearance-none <?php echo $errors->first('room') ? 'is-invalid' : ''; ?>">
                                                ...
                                                <div class="col-md-6 form-group">
                                                    ...
                                                    <div class="input-group">
                                                        <input placeholder="Email" name="email" type="text"
                                                            class="form-control input-email <?php echo $errors->first('email') ? 'is-invalid' : ''; ?>"
                                                            value="{{ Auth::user()->email ? Auth::user()->email : old('email') }}">
                                                    </div>
                                                    <p class="text-danger m-0 error-email">{{ $errors->first('email') }}
                                                    </p>
                    ...
                    <div class="osahan-cart-item rounded rounded shadow-sm overflow-hidden bg-white sticky_sidebar">
                        ...
                        <div class="bg-white p-3 py-3 border-bottom clearfix">
                            <div class="input-group-sm mb-2 input-group">
                                <input disabled class="form-control code-voucher" name="voucher"
                                    placeholder="Xem voucher khả dụng">
                                <div class="input-group-append">
                                    <div data-toggle="modal" data-target="#modalVouchers" class="btn btn-primary"><i
                                            class="feather-percent"></i> Mã giảm giá</div>
                                </div>
                            </div>
                        ...
                        <div class="bg-white p-3 clearfix border-bottom">
                            ...
                            <p class="mb-1 text-success">Giảm voucher<span
                                    class="float-right text-success discount-voucher">{{ number_format(0) }}
                                    <sup>đ</sup></span></p>
                            <hr>
                            ...
                            <div class="form-group mt-1 d-flex align-items-center">
                                <input type="radio" checked name="checkin" value="pay" id="checkin">
                                <label for="checkin" class="m-0 mx-1">Thanh toán khi nhận hàng</label>
                                <input type="radio" name="checkin" value="vnpay" id="checkinvnpay">
                                <label for="checkinvnpay" class="m-0 mx-1">Thanh toán Vnpay</label>
                            </div>
                        </div>
                        <div class="p-3">
                            <button class="btn btn-success btn-block btn-lg" name="redirect" id="text-button">Đặt hàng<i
    ...
    <div class="modal fade" id="modalVouchers" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
                        ...
                        <div class="filter filter-voucher">
                            ...
                            @foreach ($vouchers as $voucher)
                                <div class="custom-control border-bottom px-0  custom-radio">
                                    <input type="radio" id="customRadio{{ $voucher->id }}"
                                        value="{{ $voucher->code }}" name="voucher_code" class="custom-control-input">
                                    <label class="custom-control-label py-3 w-100 px-3"
                                        for="customRadio{{ $voucher->id }}">Mã <b
                                            style="color: red;">{{ $voucher->code }}</b> (giảm
...
<script>
    $(document).on('change', 'input[name=checkin]', function() {
        var value = $('input[name=checkin]:checked').val();
        if (value == 'pay') {
            $('#text-button').html('Thanh toán <i class="feather-arrow-right"></i>')
            $('form').attr('action', '{!! route('order-group-checkout') !!}')
        } else {
            $('#text-button').html('Thanh toán Vnpay <i class="feather-arrow-right"></i>')
            $('form').attr('action', '{!! route('vnpay_payment_group') !!}')
        }
    })
</script>
